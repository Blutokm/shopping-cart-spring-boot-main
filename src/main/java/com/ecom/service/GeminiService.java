...
public class GeminiService {
	...
	public ChatResponseDTO chat(ChatRequestDTO request) {
		...
		for (int attempt = 0; attempt < maxRetries; attempt++) {
			try {
				...
				HttpRequest httpRequest = HttpRequest.newBuilder().uri(URI.create(fullUrl))
				...
				if (response.statusCode() == 200) {
					...
					System.err.println("[GeminiService] Server Error " + response.statusCode() + " (attempt "
				...
				System.err.println("[GeminiService] Timeout (attempt " + (attempt + 1) + "/" + maxRetries + "): "
						+ e.getMessage());
				...
				if (attempt < maxRetries - 1) {
					...
					return ChatResponseDTO.error("Kết nối bị timeout. Vui lòng thử lại sau.");
				}

			} catch (com.fasterxml.jackson.core.JsonParseException e) {
				System.err.println("[GeminiService] JSON Parse Error: " + e.getMessage());
				e.printStackTrace();
				return ChatResponseDTO.error("Lỗi định dạng dữ liệu từ API.");

			} catch (Exception e) {
				System.err.println("[GeminiService] Unexpected Error (attempt " + (attempt + 1) + "/" + maxRetries
						+ "): " + e.getMessage());
				e.printStackTrace();
				...
				if (attempt < maxRetries - 1) {
					...
					return ChatResponseDTO.error("Đã xảy ra lỗi bất ngờ. Vui lòng thử lại.");
				}
			}
		}

		return ChatResponseDTO.error("Không thể kết nối sau " + maxRetries + " lần thử.");
	}
	...
	private String buildSystemInstruction(ChatRequestDTO request) {
		...
		if (storePhone != null && !storePhone.isBlank())
			sb.append("- Hotline: ").append(storePhone).append("\n");
		if (storeEmail != null && !storeEmail.isBlank())
			sb.append("- Email: ").append(storeEmail).append("\n");
	...
	private String buildGeminiRequestBody(String systemInstruction, List<ChatMessageDTO> messages) throws Exception {
		...
		for (ChatMessageDTO msg : messages) {
			...
			String role = msg.getRole() == null ? "user" : msg.getRole();
